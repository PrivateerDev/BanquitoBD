...
export default function Transferencia() {
  ...
  const validarPaso = () => {
    ...
    if (paso === 1) {
      if (!form.cuentaDestino || form.cuentaDestino.length !== 18) err.cuentaDestino = 'CLABE debe tener exactamente 18 dígitos'
      else if (!/^[0-9]{18}$/.test(form.cuentaDestino)) err.cuentaDestino = 'La CLABE solo debe contener números (sin letras ni espacios)'
    ...
    if (paso === 2) {
      ...
      try {
        ...
        if (data?.campos) setErrores(data.campos)
          ...
          // El backend devuelve { campos: { cuentaDestino: '...' } } cuando falla @Pattern
          if (data?.campos) setErrores(data.campos)
  ...
  return (
    <div className="p-8">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Actor: Ejecutivo (tblempleado) · Objeto: tbltransaccion · canal=SUCURSAL */}
        <TrazabilidadBar
          codes={['RF-CLI-02', 'RNF-CLI-02', 'RNF-CLI-03', 'CU-CLI-02']}
          label="tbltransaccion · tipoop=SPEI · otpvalidado=1"
        />
        {/* Stepper */}
          ...
          {paso === 1 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-1">💡 Demo: <strong className="font-mono">032180000000000099</strong> — solo dígitos, sin letras</p>
