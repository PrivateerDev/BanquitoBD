...
export default function Aclaracion() {
  const { id } = useParams()  // tblcliente.idcliente — reemplaza ID_CLIENTE=1 hardcodeado
  const ID_CLIENTE = Number(id)  // alias para no romper el resto del código
  ...
  return (
    <div className="p-6">
      ...
      {/* Actor: Ejecutivo (tblempleado) · Objeto: tblaclaracion → tbltransaccion → tblcliente */}
      <TrazabilidadBar
        codes={['RF-CLI-06', 'CU-CLI-05']}
        label={`tblaclaracion · idcliente=${id} · plazo 45 días CONDUSEF`}
      />
